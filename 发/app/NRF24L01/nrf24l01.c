...
u8 TX_ADDRESS[TX_ADR_WIDTH] = {0x13, 0x14, 0x52, 0x05, 0x20}; //自己的地址
u8 RX_ADDRESS[RX_ADR_WIDTH] = {0x13, 0x14, 0x52, 0x05, 0x21}; //对方的地址
...
u8 NRF24L01_Read_Buf(u8 reg, u8 *pBuf, u8 len)
{
	...
	for (u8_ctr = 0; u8_ctr < len; u8_ctr++)
		pBuf[u8_ctr] = NRF24L01_ReadWriteByte(0XFF); //读出数据
...
u8 NRF24L01_Write_Buf(u8 reg, u8 *pBuf, u8 len)
{
	...
	status = NRF24L01_ReadWriteByte(reg); //发送寄存器值(位置),并读取状态值
	for (u8_ctr = 0; u8_ctr < len; u8_ctr++)
		NRF24L01_ReadWriteByte(*pBuf++); //写入数据
	NRF24L01_CSN = 1;									 //关闭SPI传输
	return status;										 //返回读到的状态值
}
